...
const loginWithUpdatedPassword = async () => {
    ...
    try {
        const response = await axios.post(`${URI}/auth/v1/login`, data, config);
        console.log(
            '✅ Login with updated password successful:',
            response.data,
        );
...
const loginAfterDeletion = async () => {
    ...
    try {
        const response = await axios.post(`${URI}/auth/v1/login`, data, config);
        console.log(
            '❌ Login after deletion should have failed but succeeded:',
            response.data,
        );
        return false;
    } catch (error: any) {
        if (axios.isAxiosError(error)) {
            console.log(
                '✅ Login after deletion correctly failed:',
                error.response?.data || error.message,
            );
            return true;
        } else {
...
const testInvalidLogin = async () => {
    ...
    try {
        const response = await axios.post(`${URI}/auth/v1/login`, data, config);
        console.log(
            '❌ Invalid login should have failed but succeeded:',
            response.data,
        );
        return false;
    } catch (error: any) {
        if (axios.isAxiosError(error)) {
            console.log(
                '✅ Invalid login correctly failed:',
                error.response?.data || error.message,
            );
            return true;
        } else {
...
const testUnauthorizedUpdate = async () => {
    ...
    try {
        const response = await axios.put(`${URI}/auth/v1/update`, data, config);
        console.log(
            '❌ Unauthorized update should have failed but succeeded:',
            response.data,
        );
        return false;
    } catch (error: any) {
        if (axios.isAxiosError(error)) {
            console.log(
                '✅ Unauthorized update correctly failed:',
                error.response?.data || error.message,
            );
            return true;
        } else {
