...
export default class UpstoxManager {
    ...
    public static async deleteUserSubscription(userId: string) {
        if (!userId) {
            throw new Error(
                'User Id is required to delete the subscription schema',
            );
        }
    ...
    private async getWsAuthURL() {
        if (!this.access_code) {
            throw new Error(
                'Access Code is required. Can not generate the Websocket Authorization URL',
            );
        }
        const config: AxiosRequestConfig = {
            headers: {
        ...
        const Body: Record<string, Record<string, string>> = {
            tokenURL: {
                code:
                    code ??
                    (() => {
                        throw new Error('Missing CODE. CODE IS REQUIRED');
                    })(),
